...
namespace RenderHeads.Media.AVProVideo
{
    ...
    public sealed class WebGLMediaPlayer : BaseMediaPlayer
    {
        ...
        private static extern bool AVPPlayerInsertVideoElement(
            string path,
            int[] idValues,
            int externalLibrary
        );
        ...
        [DllImport("__Internal")]
        private static extern void AVPPlayerFetchVideoTexture(
            int player,
            IntPtr texture,
            bool init
        );
        ...
        public override bool OpenMedia(
            string path,
            long offset,
            string httpHeader,
        ...
        private void CreateTexture()
        {
            ...
            _texture = Texture2D.CreateExternalTexture(
                ...
                false,
                false,
                _cachedTextureNativePtr
            );
            if (_useTextureMips)
            {
                Debug.LogWarning(
                    "[AVProVideo] Texture Mips not yet implemented in this WebGL rendering path"
                );
            }
            ...
            if (_playerIndex >= 0)
            {
                ...
                if (AVPPlayerReady(_playerIndex))
                {
                    ...
                    if (AVPPlayerHasVideo(_playerIndex))
                    {
                        ...
                        if (
                            _texture != null
                            && (_texture.width != _width || _texture.height != _height)
                        )
                        {
                            DestroyTexture();
                        }
        ...
        internal override TrackBase InternalGetTrackInfo(
            TrackType trackType,
            int trackIndex,
            ref bool isActiveTrack
        )
        {
