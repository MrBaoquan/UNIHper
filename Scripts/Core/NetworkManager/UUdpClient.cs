...
public class UUdpClient : USocket
{
    ...
    public UUdpClient():base(NetProtocol.Udp){
        udpClient = new Socket(AddressFamily.InterNetwork, SocketType.Dgram, ProtocolType.Udp);
    }

    public UUdpClient(string InLocalIP, int InLocalPort, UNetMsgReceiver messageReceiver):this()
    {
    ...
    {
        if(udpClient==null || InData==null) return -1;
    ...
    {
        int _retCode = -1;
        if(udpClient==null || InData==null) return _retCode;
        if(!udpClient.EnableBroadcast){
            EnableBroadcast();
        }
        try
        {
            _retCode = udpClient.SendTo(InData, InData.Length, SocketFlags.None, new IPEndPoint(IPAddress.Broadcast, InPort));
        }
