...
namespace RenderHeads.Media.AVProVideo.Demos
{
    public class MediaPlayerUI : MonoBehaviour
    {
        ...
        [Header("Options")]
        [SerializeField]
        float _keyVolumeDelta = 0.05f;

        [SerializeField]
        float _jumpDeltaTime = 5f;
        ...
        [SerializeField]
        float _userInactiveDuration = 1.5f;

        [SerializeField]
        bool _useAudioFading = true;

        [Header("Keyboard Controls")]
        [SerializeField]
        bool _enableKeyboardControls = true;

        [SerializeField]
        KeyCode KeyVolumeUp = KeyCode.UpArrow;

        [SerializeField]
        KeyCode KeyVolumeDown = KeyCode.DownArrow;

        [SerializeField]
        KeyCode KeyTogglePlayPause = KeyCode.Space;

        [SerializeField]
        KeyCode KeyToggleMute = KeyCode.M;

        [SerializeField]
        KeyCode KeyJumpForward = KeyCode.RightArrow;

        [SerializeField]
        KeyCode KeyJumpBack = KeyCode.LeftArrow;
        ...
        [SerializeField]
        MediaPlayer _thumbnailMediaPlayer = null;

        [SerializeField]
        RectTransform _timelineTip = null;
        ...
        [SerializeField]
        Text _textMediaName = null;

        [SerializeField]
        Text _textTimeDuration = null;

        [SerializeField]
        Slider _sliderVolume = null;

        [SerializeField]
        Button _buttonPlayPause = null;

        [SerializeField]
        Button _buttonVolume = null;

        [SerializeField]
        Button _buttonSubtitles = null;

        [SerializeField]
        Button _buttonOptions = null;

        [SerializeField]
        Button _buttonTimeBack = null;

        [SerializeField]
        Button _buttonTimeForward = null;

        [SerializeField]
        RawImage _imageAudioSpectrum = null;

        [SerializeField]
        GameObject _optionsMenuRoot = null;

        [SerializeField]
        HorizontalSegmentsPrimitive _segmentsSeek = null;

        [SerializeField]
        HorizontalSegmentsPrimitive _segmentsBuffered = null;

        [SerializeField]
        HorizontalSegmentsPrimitive _segmentsProgress = null;
        ...
        private struct UserInteraction
        {
            ...
            public static bool IsUserInputThisFrame()
            {
                ...
                bool mouseInput = (
                    Input.mousePresent
                    && (
                        Input.mousePosition != _previousMousePos
        ...
        private void SetupPlayPauseButton()
        {
            if (_buttonPlayPause)
            {
                ...
                _playPauseMaterial = DuplicateMaterialOnImage(
                    _buttonPlayPause.GetComponent<Image>()
                );
            }
        }
        ...
        private void SetupSubtitlesButton()
        {
            if (_buttonSubtitles)
            {
                ...
                _subtitlesMaterial = DuplicateMaterialOnImage(
                    _buttonSubtitles.GetComponent<Image>()
                );
            }
        }
            ...
            if (_mediaPlayer && _mediaPlayer.TextTracks != null)
            {
                if (_mediaPlayer.TextTracks.GetTextTracks().Count > 0)
                {
                    ...
                    else
                    {
                        ...
                        _mediaPlayer.TextTracks.SetActiveTextTrack(
                            _mediaPlayer.TextTracks.GetTextTracks()[0]
                        );
                    }
                }
            }
        ...
        void Update()
        {
            ...
            if (_mediaPlayer.Info != null)
            {
                        ...
                        if (UserInteraction.IsUserInputThisFrame())
                        {
                            ...
                            Bounds bounds =
                                RectTransformUtility.CalculateRelativeRectTransformBounds(
                                    this._sliderTime.GetComponent<RectTransform>()
                                );
                            ...
                            if (
                                _thumbnailMediaPlayer != null
                                && _thumbnailMediaPlayer.Control != null
                            )
                            {
                ...
                if (_buttonSubtitles)
                {
                    _buttonSubtitles.gameObject.SetActive(
                        _mediaPlayer.TextTracks.GetTextTracks().Count > 0
                    );
                }
                ...
                if (_textMediaName)
                {
                    ...
                    {
                        resolutionName = Helper.GetFriendlyResolutionName(
                            _mediaPlayer.Info.GetVideoWidth(),
                            _mediaPlayer.Info.GetVideoHeight(),
                            _mediaPlayer.Info.GetVideoFrameRate()
                        );
                    }
                ...
                if (_segmentsBuffered)
                {
                    ...
                    if (times.Count > 0 && timelineRange.duration > 0.0)
                    {
                        ...
                        {
                            ranges[i * 2 + 0] = Mathf.Max(
                                0f,
                                (float)(
                                    (times[i].StartTime - timelineRange.startTime)
                                    / timelineRange.duration
                                )
                            );
                            ranges[i * 2 + 1] = Mathf.Min(
                                1f,
                                (float)(
                                    (times[i].EndTime - timelineRange.startTime)
                                    / timelineRange.duration
                                )
                            );
                        }
                ...
                if (_segmentsProgress)
                {
                    ...
                    if (times.Count > 0 && timelineRange.Duration > 0.0)
                    {
                        ranges = new float[2];
                        double x1 =
                            (times.MinTime - timelineRange.startTime) / timelineRange.duration;
                        double x2 = (
                            (_mediaPlayer.Control.GetCurrentTime() - timelineRange.startTime)
                            / timelineRange.duration
                        );
        ...
        void OnGUI()
        {
            ...
            if (
                _mediaPlayer.PlatformOptionsWindows.bufferedFrameSelection
            ...
            )
            {
                ...
                if (bufferedDisplay != null)
                {
                    ...
                    GUILayout.Label(
                        "Display Timstamp: " + _mediaPlayer.TextureProducer.GetTextureTimeStamp()
                    );
                }
            }
#endif
