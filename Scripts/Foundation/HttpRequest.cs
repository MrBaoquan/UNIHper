...
namespace UNIHper {
    ...
    public static class HttpRequest {
        ...
        public static IObservable<string> Get (string url) {
            ...
            return Observable.FromCoroutine<string> ((observer, cancellationToken) => GetCore (url, observer, cancellationToken))
                .Select (_response => _response);
        }

        static IEnumerator GetCore (string url, IObserver<string> observer, CancellationToken cancellationToken) {
        ...
        public static IObservable<HttpResponse> Download (string url, string savePath) {
            return Observable.FromCoroutine<HttpResponse> ((observer, cancellationToken) => DownloadCore (url, savePath, true, observer, cancellationToken));
        }

        ///
        /// 注意: 在安卓平台有内存限制，不能下载大文件      TODO: 大文件支持
        // Note: IObserver's basic scheme is "OnNext* (OnError | Oncompleted)?" 
        static IEnumerator DownloadCore (string url, string savePath, bool updateSpeed, IObserver<HttpResponse> observer, CancellationToken cancellationToken) {
