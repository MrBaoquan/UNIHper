...
namespace UNIHper {
    ...
    public class UTcpClient : USocket {
        ...
        public UTcpClient Connect () {
            ...
            connectDisposable = Observable.Interval (TimeSpan.FromMilliseconds (3000))
                .Where ((_1, _2) => clientReuse && !Connected).Subscribe (_3 => {
        ...
        private void doDisconnect (bool bReuse = false) {
            ...
            if (connected) {
                connected = false;
                var _disconnectedEvent = new UNetDisconnectedEvent { RemoteIP = remoteEndPoint.Address.ToString (), RemotePort = remoteEndPoint.Port };
