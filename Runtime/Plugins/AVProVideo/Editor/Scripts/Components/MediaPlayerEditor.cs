    ...
    public partial class MediaPlayerEditor : UnityEditor.Editor
    {
        ...
        internal static void DeleteSettings()
        {
            ...
            EditorPrefs.DeleteKey(
                AnimCollapseSection.GetPrefName(GetPlatformButtonContent(Platform.Windows).text)
            );
            EditorPrefs.DeleteKey(
                AnimCollapseSection.GetPrefName(GetPlatformButtonContent(Platform.MacOSX).text)
            );
            EditorPrefs.DeleteKey(
                AnimCollapseSection.GetPrefName(GetPlatformButtonContent(Platform.Android).text)
            );
            EditorPrefs.DeleteKey(
                AnimCollapseSection.GetPrefName(GetPlatformButtonContent(Platform.iOS).text)
            );
            EditorPrefs.DeleteKey(
                AnimCollapseSection.GetPrefName(GetPlatformButtonContent(Platform.tvOS).text)
            );
            EditorPrefs.DeleteKey(
                AnimCollapseSection.GetPrefName(GetPlatformButtonContent(Platform.WindowsUWP).text)
            );
            EditorPrefs.DeleteKey(
                AnimCollapseSection.GetPrefName(GetPlatformButtonContent(Platform.WebGL).text)
            );
        }
        ...
        private void CreateSections()
        {
            ...
            _sectionMediaInfo = new AnimCollapseSection(
                "Media Info",
                false,
                false,
                OnInspectorGUI_MediaInfo,
                this,
                mediaInfoColor
            );
            _sectionDebug = new AnimCollapseSection(
                "Debug",
                false,
                true,
            ...
            _sectionSettings = new AnimCollapseSection(
                "Settings",
                false,
                true,
            ...
            _sectionAboutHelp = new AnimCollapseSection(
                "About / Help",
                false,
                false,
            ...
            _settingSections.Add(
                new AnimCollapseSection(
                    "Source",
                    false,
                    true,
                    OnInspectorGUI_Source,
                    this,
                    sourceColor
                )
            );
            ...
            _sectionDevModeState = new AnimCollapseSection(
                "State",
                false,
                false,
                OnInspectorGUI_DevMode_State,
                this,
                Color.white
            );
            _sectionDevModeTexture = new AnimCollapseSection(
                "Texture",
                false,
                false,
                OnInspectorGUI_DevMode_Texture,
                this,
                Color.white
            );
            _sectionDevModePlaybackQuality = new AnimCollapseSection(
                "Presentation Quality",
                false,
                false,
                OnInspectorGUI_DevMode_PresentationQuality,
                this,
                Color.white
            );
            _sectionDevModeHapNotchLCDecoder = new AnimCollapseSection(
                "Hap/NotchLC Decoder",
                false,
                false,
                OnInspectorGUI_DevMode_HapNotchLCDecoder,
                this,
                Color.white
            );
            _sectionDevModeBufferedFrames = new AnimCollapseSection(
                "Buffers",
                false,
                false,
                OnInspectorGUI_DevMode_BufferedFrames,
                this,
                Color.white
            );
        }
        ...
        private void ShowImportantMessages()
        {
            ...
            if (_isTrialVersion)
            {
                ...
                if (Application.isPlaying)
                {
                    ...
                    MediaPlayer media = (this.target) as MediaPlayer;
                    message =
                        "The watermark is the horizontal bar that moves vertically and the small 'AVPRO TRIAL' text.";
                    if (
                        media.Info != null
                        && media.Info.GetPlayerDescription().Contains("MF-MediaEngine-Hardware")
                    )
                    {
                        message =
                            "The watermark is the RenderHeads logo that moves around the image.";
                    }
                ...
                EditorHelper.IMGUI.BeginWarningTextBox(
                    "AVPRO VIDEO - TRIAL WATERMARK",
                    message,
                    Color.yellow,
                    Color.yellow,
                    Color.white
            ...
            // Warning about not using multi-threaded rendering
            {
                    ...
                    EditorUserBuildSettings.selectedBuildTargetGroup == BuildTargetGroup.Android)
                {
                    ...
                    showWarningMT = !UnityEditor.PlayerSettings.GetMobileMTRendering(
                ...
                if (showWarningMT)
                {
                    EditorHelper.IMGUI.WarningTextBox(
                        "Performance Warning",
                        "Deploying to Android with multi-threaded rendering disabled is not recommended.  Enable multi-threaded rendering in the Player Settings > Other Settings panel.",
                        Color.yellow,
                        Color.yellow,
                        Color.white
                    );
            ...
            )
            {
                EditorHelper.IMGUI.WarningTextBox(
                    ...
                    "Direct3D 12 is not supported until Unity 2019.3",
                    Color.yellow,
                    Color.yellow,
                    Color.white
                );
...
#if UNITY_2018_1_OR_NEWER
            {
                if (EditorUserBuildSettings.selectedBuildTargetGroup == BuildTargetGroup.Android)
                {
                    ...
                    {
                        UnityEngine.Rendering.GraphicsDeviceType[] devices =
                            UnityEditor.PlayerSettings.GetGraphicsAPIs(BuildTarget.Android);
                    ...
                    if (showWarningVulkan)
                    {
                        EditorHelper.IMGUI.WarningTextBox(
                            "Compatibility Warning",
                            "Vulkan graphics API is not supported.  Please go to Player Settings > Android > Auto Graphics API and remove Vulkan from the list.  Only OpenGL 2.0 and 3.0 are supported on Android.",
                            Color.yellow,
                            Color.yellow,
                            Color.white
                        );
        ...
        private void OnInspectorGUI_Main()
        {
            ...
            EditorGUILayout.PropertyField(
                ...
                new GUIContent(
                    "Persistent",
                    "Use DontDestroyOnLoad so this object isn't destroyed between level loads"
                )
            );
            ...
            if (_propForceFileFormat != null)
            {
                GUIContent label = new GUIContent(
                    "Force File Format",
                    "Override automatic format detection when using non-standard file extensions"
                );
                _propForceFileFormat.enumValueIndex = EditorGUILayout.Popup(
                    label,
                    _propForceFileFormat.enumValueIndex,
                    _fileFormatGuiNames
                );
            }
        ...
        public override bool RequiresConstantRepaint()
        {
            ...
            if (
                media != null
                && media.Control != null
                && media.isActiveAndEnabled
                && media.Info.GetDuration() > 0.0
            )
            {
