    ...
    public class UVideoPlayer : MonoBehaviour {
        ...
        public UVideoPlayer BuildRender (int Width = -1, int Height = -1, bool bAutoRender = true) {
            ...
            if (rectTransform) { // 如果是UI组件 则渲染为UI组件的尺寸
                ...
                if (Height == -1) Height = (int) rectTransform.rect.height;
            } else { // 渲染为屏幕大小的尺寸
        ...
        public void Play (string InUrl, VideoPlayer.EventHandler OnReachEndHandler = null, int loop = -1, float StartTime = 0, float InEndTime = 0) {
