...
namespace RenderHeads.Media.AVProVideo.Editor
{
    ...
    {
#if UNITY_EDITOR_OSX
        internal const string MediaFileExtensions =
            "mp4,m4v,mov,mpg,avi,mp3,m4a,aac,ac3,au,aiff,caf,wav,m3u8";
#else
        internal const string MediaFileExtensions =
            "Media Files;*.mp4;*.mov;*.m4v;*.avi;*.mkv;*.ts;*.webm;*.flv;*.vob;*.ogg;*.ogv;*.mpg;*.wmv;*.3gp;*.mxf;Audio Files;*wav;*.mp3;*.mp2;*.m4a;*.wma;*.aac;*.au;*.flac;*.m3u8;*.mpd;*.ism;";
#endif
        ...
        private void OnInspectorGUI_Source()
        {
            ...
            //if (!Application.isPlaying)
            {
                ...
                if (GUILayout.Button("Load"))
                {
                    if (mediaPlayer.MediaSource == MediaSource.Path)
                    {
                        mediaPlayer.OpenMedia(
                            mediaPlayer.MediaPath.PathType,
                            mediaPlayer.MediaPath.Path,
                ...
                if (
                    EditorGUIUtility.GetObjectPickerControlID() == 100
                    && Event.current.commandName == "ObjectSelectorClosed"
                )
                {
                    MediaReference mediaRef = (MediaReference)
        ...
        internal static void ShowFileWarningMessages(
        ...
        )
        {
            ...
            else
            {
                ...
                // Test file extensions
                {
                    ...
                    if (isPlatformAndroid && isExtensionMOV)
                    {
                        EditorHelper.IMGUI.NoticeBox(
                            MessageType.Warning,
                            "MOV file detected. Android doesn't support MOV files, you should change the container file."
                        );
                    }
                    if (isPlatformAndroid && isExtensionAVI)
                    {
                        EditorHelper.IMGUI.NoticeBox(
                            MessageType.Warning,
                            "AVI file detected. Android doesn't support AVI files, you should change the container file."
                        );
                    }
                    if (isPlatformAndroid && isExtensionMKV)
                    {
                        EditorHelper.IMGUI.NoticeBox(
                            MessageType.Warning,
                            "MKV file detected. Android doesn't support MKV files until Android 5.0."
                        );
                    }
                    if (isPlatformIOS && isExtensionAVI)
                    {
                        EditorHelper.IMGUI.NoticeBox(
                            MessageType.Warning,
                            "AVI file detected. iOS doesn't support AVI files, you should change the container file."
                        );
                    }
                }
                ...
                if (fullPath.Contains("://"))
                {
                    if (fullPath.ToLower().Contains("rtmp://"))
                    {
                        EditorHelper.IMGUI.NoticeBox(
                            MessageType.Warning,
                            "RTMP protocol is not supported by AVPro Video, except when Windows DirectShow is used with an external codec library (eg LAV Filters)"
                        );
                    }
                    if (fullPath.ToLower().Contains("youtube.com/watch"))
                    {
                        EditorHelper.IMGUI.NoticeBox(
                            MessageType.Warning,
                            "YouTube URL detected. YouTube website URL contains no media, a direct media file URL (eg MP4 or M3U8) is required.  See the documentation FAQ for YouTube support."
                        );
                    }
                    if (mediaPath.PathType != MediaPathType.AbsolutePathOrURL)
                    {
                        EditorHelper.IMGUI.NoticeBox(
                            MessageType.Warning,
                            "URL detected, change location type to URL?"
                        );
                    }
                    else
                    {
                        ...
                        if (isPlatformIOS || isPlatformTVOS)
                        {
                            ...
                            )
                            {
                                EditorHelper.IMGUI.NoticeBox(
                                    MessageType.Warning,
                                    "Starting with iOS 9 'allow HTTP downloads' must be enabled for HTTP connections (see Player Settings)"
                                );
                            }
                        }
#if UNITY_ANDROID
                        if (fullPath.StartsWith("http://"))
                        {
                            EditorHelper.IMGUI.NoticeBox(
                                MessageType.Warning,
                                "Starting with Android 8 unsecure HTTP is not allowed by default and HTTPS must be used, unless a custom cleartext security policy is assigned"
                            );
                        }
#endif
                        ...
                        if (isPlatformAndroid && !PlayerSettings.Android.forceInternetPermission)
                        {
                            EditorHelper.IMGUI.NoticeBox(
                                MessageType.Warning,
                                "You need to set 'Internet Access' to 'require' in your Player Settings for Android builds when using URLs"
                            );
                        }
                        ...
                        )
                        {
                            ...
                            )
                            {
                                EditorHelper.IMGUI.NoticeBox(
                                    MessageType.Warning,
                                    "You need to set 'InternetClient' capability in your Player Settings when using URLs"
                                );
                            }
                        }
                    }
                ...
                else
                {
                    ...
                    )
                    {
                        EditorHelper.IMGUI.NoticeBox(
                            MessageType.Warning,
                            "Absolute path detected, change location to Absolute path?"
                        );
                    }
#endif
                    ...
                    if (isPlatformAndroid && !PlayerSettings.Android.forceSDCardPermission)
                    {
                        EditorHelper.IMGUI.NoticeBox(
                            MessageType.Warning,
                            "You may need to access the local file system you may need to set 'Write Access' to 'External(SDCard)' in your Player Settings for Android"
                        );
                    }
                            ...
                            if (!Application.isPlaying)
                            {
                                ...
                                if (!string.IsNullOrEmpty(folderPath))
                                {
                                    ...
                                    if (!caseMatch)
                                    {
                                        EditorHelper.IMGUI.NoticeBox(
                                            MessageType.Warning,
                                            "File found but case doesn't match"
                                        );
                                    }
                                }
                            }
            ...
            if (
                mediaPath != null
                && mediaPath.PathType == MediaPathType.RelativeToStreamingAssetsFolder
            )
            {
                    ...
                    if (checkAndroidFileSize)
                    {
                        try
                        {
                            ...
                            if (info != null && info.Length > (1024 * 1024 * 512))
                            {
                                EditorHelper.IMGUI.NoticeBox(
                                    MessageType.Warning,
                                    "Using this very large file inside StreamingAssets folder on Android isn't recommended.  Deployments will be slow and mapping the file from the StreamingAssets JAR may cause storage and memory issues.  We recommend loading from another folder on the device."
                                );
                            }
                        }
