...
public class FileDialog : UIBase
{
    ...
    {
        EnableDragMove();
        this.Get<UPanel>().OnCloseAsObservable()
            .Subscribe(_=>{
                Hide();
            });
        ...
        _openFileButton.OnClickAsObservable()
        ...
        _saveFileButton.OnClickAsObservable()
            .Subscribe(_=>{
        ...
        _fileName.OnValueChangedAsObservable()
            .Subscribe(_value=>{
                    ...
                    if(!_containFile){
                        ...
                        Observable.NextFrame().Subscribe(_=>{
                            this.Get<ScrollRect>("scroll_fileList")
                                .ScrollToCenter(fileItems[_value].GetComponent<RectTransform>(),RectTransform.Axis.Vertical);
                        });
    ...
    private void handleFilterFiles(string InFilter)
    {
        ...
        var _invalid = fileItems.Keys.Where(_key=>!_key.ToLower().Contains(InFilter)).ToList();
        _invalid.ForEach(_key=>{
                fileItems[_key].gameObject.SetActive(false);
            });

        fileItems.Keys.Except(_invalid)
    ...
    public IObservable<string> ReadFile(string InDir, string SearchPattern="*.*")
    {
        ...
        return Observable.FromEvent<string>(_action=>OnOpenFileHandler+=_action,_action=>OnOpenFileHandler-=_action);
    }
    ...
    public IObservable<string> SaveFile(string InDir, string SearchPattern="*.*")
    {
        ...
        syncLayout();
        LoadFilesInDirectory(InDir, SearchPattern);
        return Observable.FromEvent<string>(_action=>OnSaveFileHandler+=_action, _action=>OnSaveFileHandler-=_action);
    }
    ...
    private void syncFileList(DirectoryInfo InDirectoryInfo, string SearchPattern="*.*")
    {
        ...
        fileInfos = InDirectoryInfo.GetFiles(SearchPattern).ToDictionary(_fileInfo=>_fileInfo.Name, _fileInfo=>_fileInfo);

        var _fileItem = Managements.Resource.GetRes<GameObject>("FileItem");
        fileInfos.Keys.ToList()
            .ForEach(_fileName=>{
                ...
                _newFileItem.GetComponent<Button>().OnClickAsObservable().Subscribe(_=>{
    ...
    private void selectFile(string InFileName)
    {
        ...
        if(!fileItems.Keys.Contains(InFileName)) return;
